...
output2 = tf.keras.layers.Dense(1)(h2)

model = tf.keras.models.Model(
    inputs=[input_wide, input_deep], outputs=[output1, output2]
)

model.summary()

logdir = os.path.join('wide_deep_multi_output_callbacks')
if not os.path.exists(logdir): os.mkdir(logdir)
output_model_file = os.path.join(logdir, 'wide_deep_multi_output_model.h5')
...
history = model.fit(
    [x_train_wide, x_train_deep], [y_train, y_train], epochs=64,
